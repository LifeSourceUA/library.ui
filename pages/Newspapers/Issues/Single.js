...
class Single extends Component {
    ...
    loadItem = async () => {
        ...
        const newspaper = await response[0].json();
    ...
    handleSave = async () => {
        ...
        if (action === 'create') {
            Router.pushRoute('newspapers-issues-single', {
                periodicalId,
                issueId: result.id
            });
        } else {
            Router.pushRoute('newspapers-issues-list', {
                periodicalId
            });
        }
    };
    ...
    renderYearNums = () => {
        const { issue, newspaper, action } = this.state;
        ...
        const onClick = (value) => {
            return () => {
                ...
                const numberTotal = newspaper.issues.reduce((result, item) => {
        ...
        const usedNumbers = newspaper.issues.reduce((result, item) => {
